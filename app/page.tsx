      ...
      <div className="w-full max-w-md bg-[#FFF9F0] h-full flex flex-col relative shadow-2xl">
        ...
        {resultPopup && (
          <div className="absolute inset-0 bg-black/70 z-50 flex items-center justify-center p-6 backdrop-blur-sm cursor-pointer" onClick={() => setResultPopup(null)}>
            <div className="bg-white p-6 rounded-3xl shadow-2xl text-center transform scale-105 transition-transform w-full max-w-[280px] animate-bounce-short">
        ...
        {/* 상단 헤더 (고정) */}
        <div className="shrink-0 flex justify-between items-center p-4 pt-6 z-10">
          <h1 className="text-xl font-black text-gray-900 tracking-tight">{lang === 'ko' ? '비밀번호를 찾아라 🔐' : 'Crack the Code 🔐'}</h1>
          <button onClick={() => setLang(lang === 'ko' ? 'en' : 'ko')} className="px-3 py-1 bg-white border-2 border-gray-300 rounded-full text-[10px] font-bold shadow-sm active:bg-gray-100">
            {lang === 'ko' ? '🇰🇷 KO' : '🇺🇸 EN'}
          </button>
        </div>

        {/* 중간 내역 영역 (여기만 스크롤됨) */}
        <div className="flex-1 overflow-y-auto px-4 pb-2 scrollbar-hide z-10">
          ...
          <div className="bg-white rounded-xl p-3 min-h-[150px] shadow-sm border border-gray-200 mb-2">
            {history.length === 0 ? (
        ...
        <div className="shrink-0 bg-white rounded-t-2xl border-t border-gray-200 p-4 pb-16 shadow-[0_-10px_20px_rgba(0,0,0,0.03)] z-10 relative">
          <div className="flex justify-between mb-3">
            ...
            <button onClick={isCleared ? resetGame : handleGuess} className={`flex-1 ml-1.5 text-white py-2.5 rounded-lg font-black text-sm shadow-md active:scale-95 transition-transform ${isCleared ? 'bg-green-500' : 'bg-gray-800'}`}>
              {isCleared ? (lang === 'ko' ? '다음 스테이지' : 'Next') : (lang === 'ko' ? '정답 확인' : 'Check')}
            </button>
          </div>
          ...
          <div className="absolute bottom-0 left-0 w-full h-[50px] bg-gray-100 flex items-center justify-center text-gray-400 text-[10px] font-bold border-t border-gray-200">
